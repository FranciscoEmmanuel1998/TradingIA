...
export class EmergentGoalEngine {
  ...
  private createHypothesis(goal: EmergentGoal): StrategyHypothesis | null {
    const symbol = goal.targetSymbols[Math.floor(Math.random() * goal.targetSymbols.length)];
  ...
  private createMomentumHypothesis(goal: EmergentGoal, symbol: string, currentPrice: number): StrategyHypothesis {
    const priceChange = Math.random() * 0.02 + 0.01; // 1-3% movimiento esperado
    const direction = Math.random() > 0.5 ? 1 : -1;
    
    return {
      id: `hyp_${Date.now()}_${Math.random().toString(36).substr(2, 5)}`,
      goalId: goal.id,
      entry: {
  ...
  private createMeanReversionHypothesis(goal: EmergentGoal, symbol: string, currentPrice: number): StrategyHypothesis {
    const reversion = Math.random() * 0.01 + 0.005; // 0.5-1.5% reversión
    const direction = Math.random() > 0.5 ? -1 : 1; // Contrario al momentum
  ...
  private createDiscoveryHypothesis(goal: EmergentGoal, symbol: string, currentPrice: number): StrategyHypothesis {
    ...
    const experimentalMove = Math.random() * 0.05 + 0.02; // 2-7% movimiento
    const direction = Math.random() > 0.5 ? 1 : -1;
  ...
  private createRandomGoal(): void {
    ...
    this.createGoal({
      ...
      targetSymbols: [symbols[Math.floor(Math.random() * symbols.length)]],
      expectedSharpe: 1.0 + Math.random() * 1.5,
      riskBudget: 0.03 + Math.random() * 0.07 // 3-10%
    });
  }
